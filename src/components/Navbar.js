...
const Navbar = () => {
  ...
  const { auth, logout } = useAuth(); // Accédez à la fonction logout

  return (
    <nav className="text-white flex justify-between items-center p-4" style={{ backgroundColor: 'rgb(22, 55, 103)' }}>
      <img src="/images/logo-myefrei-pantheon.png" alt="Logo myEfrei" className="w-32 h-auto" />
      <div className="relative">
        <button
          onClick={() => setShowPopup(!showPopup)}
          className="flex items-center space-x-2 px-4 py-2 rounded-full"
        >
          <span>{auth?.user?.firstname} {auth?.user?.name} ({auth?.role})</span> {/* Utilisez les informations de l'utilisateur */}
        ...
        {showPopup && (
          <div className="absolute right-0 mt-2 w-48 bg-white text-black shadow-lg rounded-lg p-4">
            <button
              onClick={logout} // Appelez la fonction logout
