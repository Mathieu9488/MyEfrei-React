...
export default function AdminProfesseursPage() {
  ...
  const filteredProfesseurs = sortedProfesseurs.filter((prof) =>
    `${prof.name} ${prof.firstname}`.toLowerCase().includes(search.toLowerCase())
  );

  const totalPages = Math.ceil(filteredProfesseurs.length / itemsPerPage);
  const displayedProfesseurs = filteredProfesseurs.slice(
    ...
    <div className="w-full h-screen relative">
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-xl font-bold">Gestion des professeurs</h1>
          <button
            onClick={openCreateModal}
            className="p-2 text-white rounded flex items-center"
            style={{ backgroundColor: "rgb(23, 82, 168)" }}
          >
            <Plus size={16} className="mr-2" /> Nouveau professeur
          </button>
        </div>
            ...
            <tbody>
              {displayedProfesseurs.map((professeur) => (
                <tr key={professeur.id}>
                  <td className="py-2 px-4 border">{professeur.id}</td>
                  <td className="py-2 px-4 border">{professeur.name}</td>
                  <td className="py-2 px-4 border">{professeur.firstname}</td>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded shadow-lg w-1/3">
              <h2 className="text-xl font-bold mb-4">
                {isEditing ? "Mettre à jour un professeur" : "Ajouter un professeur"}
              </h2>
              <input
                type="text"
                placeholder="Nom"
              ...
              <input
                type="password"
                placeholder={isEditing ? "Nouveau mot de passe (facultatif)" : "Mot de passe"}
